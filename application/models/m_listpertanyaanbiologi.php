...
class M_listpertanyaanbiologi extends CI_Model {

    public function __construct(){
        $this->load->database();
    }

    public function aturscorebiologi(){
        $aturscorebiologi = $this->db->query('SELECT * FROM tb_score WHERE id_score = 4');
        $datascore = $aturscorebiologi->result();
        return $datascore[0];
    }

    public function max_data(){
    	$datamax = $this->db->query('SELECT * FROM tb_listpertanyaanbiologi');
    ...
    public function tambah_historybiologi(){
		$tambah_historybiologi = array(
		...
		$tambah = $this->db->insert("tb_historybiologi", $tambah_historybiologi);
		return $tambah;
    }

    public function get_pertanyaan($idpertanyaan){       	
		$this->db->select('*');
        $this->db->from('tb_listpertanyaanbiologi');
