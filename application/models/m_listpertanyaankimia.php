...
class M_listpertanyaankimia extends CI_Model {

    public function __construct(){
        $this->load->database();
    }

    public function aturscorekimia(){
        $aturscorekimia = $this->db->query('SELECT * FROM tb_score WHERE id_score = 6');
        $datascore = $aturscorekimia->result();
        return $datascore[0];
    }

    public function max_data(){
    	$datamax = $this->db->query('SELECT * FROM tb_listpertanyaankimia');
    ...
    public function tambah_historykimia(){
		$tambah_historykimia = array(
		...
		$tambah = $this->db->insert("tb_historykimia", $tambah_historykimia);
		return $tambah;
    }

    public function get_pertanyaan($idpertanyaan){       	
		$this->db->select('*');
        $this->db->from('tb_listpertanyaankimia');
