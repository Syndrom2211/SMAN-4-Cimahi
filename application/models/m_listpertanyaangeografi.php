...
class M_listpertanyaangeografi extends CI_Model {

    public function __construct(){
        $this->load->database();
    }

    public function aturscoregeografi(){
        $aturscoregeografi = $this->db->query('SELECT * FROM tb_score WHERE id_score = 8');
        $datascore = $aturscoregeografi->result();
        return $datascore[0];
    }

    public function max_data(){
    	$datamax = $this->db->query('SELECT * FROM tb_listpertanyaangeografi');
    ...
    public function tambah_historygeografi(){
		$tambah_historygeografi = array(
		...
		$tambah = $this->db->insert("tb_historygeografi", $tambah_historygeografi);
		return $tambah;
    }

    public function get_pertanyaan($idpertanyaan){       	
		$this->db->select('*');
        $this->db->from('tb_listpertanyaangeografi');
