...
class M_listpertanyaanfisika extends CI_Model {

    public function __construct(){
        $this->load->database();
    }

    public function aturscorefisika(){
        $aturscorefisika = $this->db->query('SELECT * FROM tb_score WHERE id_score = 5');
        $datascore = $aturscorefisika->result();
        return $datascore[0];
    }

    public function max_data(){
    	$datamax = $this->db->query('SELECT * FROM tb_listpertanyaanfisika');
    ...
    public function tambah_historyfisika(){
		$tambah_historyfisika = array(
		...
		$tambah = $this->db->insert("tb_historyfisika", $tambah_historyfisika);
		return $tambah;
    }

    public function get_pertanyaan($idpertanyaan){       	
		$this->db->select('*');
        $this->db->from('tb_listpertanyaanfisika');
