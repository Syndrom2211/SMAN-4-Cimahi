...
class M_listpertanyaansejarah extends CI_Model {

    public function __construct(){
        $this->load->database();
    }

    public function aturscoresejarah(){
        $aturscoresejarah = $this->db->query('SELECT * FROM tb_score WHERE id_score = 10');
        $datascore = $aturscoresejarah->result();
        return $datascore[0];
    }

    public function max_data(){
    	$datamax = $this->db->query('SELECT * FROM tb_listpertanyaansejarah');
    ...
    public function tambah_historysejarah(){
		$tambah_historysejarah = array(
		...
		$tambah = $this->db->insert("tb_historysejarah", $tambah_historysejarah);
		return $tambah;
    }

    public function get_pertanyaan($idpertanyaan){       	
		$this->db->select('*');
        $this->db->from('tb_listpertanyaansejarah');
