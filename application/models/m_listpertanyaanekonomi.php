...
class M_listpertanyaanekonomi extends CI_Model {

    public function __construct(){
        $this->load->database();
    }

    public function aturscoreekonomi(){
        $aturscoreekonomi = $this->db->query('SELECT * FROM tb_score WHERE id_score = 9');
        $datascore = $aturscoreekonomi->result();
        return $datascore[0];
    }

    public function max_data(){
    	$datamax = $this->db->query('SELECT * FROM tb_listpertanyaanekonomi');
    ...
    public function tambah_historyekonomi(){
		$tambah_historyekonomi = array(
		...
		$tambah = $this->db->insert("tb_historyekonomi", $tambah_historyekonomi);
		return $tambah;
    }

    public function get_pertanyaan($idpertanyaan){       	
		$this->db->select('*');
        $this->db->from('tb_listpertanyaanekonomi');
