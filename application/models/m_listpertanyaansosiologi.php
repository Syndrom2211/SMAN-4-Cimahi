...
class M_listpertanyaansosiologi extends CI_Model {

    public function __construct(){
        $this->load->database();
    }

    public function aturscoresosiologi(){
        $aturscoresosiologi = $this->db->query('SELECT * FROM tb_score WHERE id_score = 7');
        $datascore = $aturscoresosiologi->result();
        return $datascore[0];
    }

    public function max_data(){
    	$datamax = $this->db->query('SELECT * FROM tb_listpertanyaansosiologi');
    ...
    public function tambah_historysosiologi(){
		$tambah_historysosiologi = array(
		...
		$tambah = $this->db->insert("tb_historysosiologi", $tambah_historysosiologi);
		return $tambah;
    }

    public function get_pertanyaan($idpertanyaan){       	
		$this->db->select('*');
        $this->db->from('tb_listpertanyaansosiologi');
